...
for(const source of old){
 ...
 assert.equal(lab.points,lab.semester===6?(lab.number<=3?3:4):lab.semester===7?2.5:10)
 assert(Math.abs(lab.points-lab.rubric.reduce((n,r)=>n+r.points,0))<1e-9)
...
assert.equal(combinations,660);assert.deepEqual(totals,{6:25,7:25,8:50})
