...
function CourseApp() {
  ...
  const [subjectAreaId, setSubjectAreaId] = useState(() => {
    const saved = Number(window.localStorage.getItem('uiabd-lab-subject-area'))
  ...
  const selectSubjectArea = (id: number) => {
    setSubjectAreaId(id)
    window.localStorage.setItem('uiabd-lab-subject-area', String(id))
  }
  ...
  if (route.kind === 'lab' && lab) return <><LabPage key={`${lab.slug}-${subjectArea.id}`} lab={resolveLab(lab,subjectArea.id)} subjectArea={subjectArea} profile={profile} onSubjectAreaChange={selectSubjectArea} /></>
    ...
    <div className="site-shell">
      ...
      <main id="main-content" tabIndex={-1}>
        ...
        <section className="student-start" aria-labelledby="archive-title">
          ...
          <ol><li><strong>Начните_здесь.html</strong> — откройте первым в браузере. Внутри: условия вашего варианта, исходные данные, пояснения, пример, шаги и самопроверка. Оформление встроено в файл, интернет не нужен.</li><li><strong>Шаблон_для_заполнения.docx</strong> — откройте в Word или другом редакторе DOCX. Заполните и сохраните отчёт.</li><li><strong>Папка «Данные»</strong> — CSV с исходными таблицами и условиями вашего варианта для работы в Excel или другом табличном редакторе. При импорте выберите UTF-8 и разделитель «точка с запятой».</li><li><strong>Папка «Стенд»</strong> — исходные SQL, JSON и конфигурации, необходимые этой работе. Инструкция запуска находится в docs/stand-start.md внутри этой папки. Для запуска СУБД и мониторинга нужна подготовленная учебная среда.</li></ol>
        ...
        <section className="catalog-section" id="labs" aria-labelledby="labs-title">
          ...
          {visibleLabs.length ? (
            <div className="lab-grid">
              {visibleLabs.map((lab) => <LabCard key={lab.slug} lab={lab} area={subjectArea} profile={profile} />)}
            </div>
          ) : (
...
function LabPage({ lab, subjectArea, profile, onSubjectAreaChange }: { lab: Lab; subjectArea: SubjectArea; profile: QualityProfile; onSubjectAreaChange: (id: number) => void }) {
  ...
  const position = labs.findIndex(item=>item.slug===lab.slug)
  const previous = labs[position-1]
  ...
  const methodology = labMethodology[lab.slug]
          ...
          <div className="lab-hero-inner">
            ...
            <div className="lab-hero-grid">
              <div className="lab-hero-copy">
                <p className="lab-eyebrow"><span className="semester-pill">{lab.semester} семестр</span><span>Лабораторная работа {lab.number}</span></p>
        ...
        <div className="lab-page-grid">
          <article className="lab-content">
            ...
            <ContentSection id="inputs" number="Б" label="Стартовый пакет" title="Пояснение к задаче по предметной области" icon={<Layers3 aria-hidden="true" />}>
              ...
              <h3>Файлы учебного стенда</h3><ul>{lab.standFiles.map(path=><li key={path}><a href={import.meta.env.BASE_URL+path} download>{path.split('/').pop()}</a></li>)}</ul><h3>Инструменты и допустимая среда</h3><Checklist items={lab.tools.map(labText)} compact />
            </ContentSection>
            ...
            <ContentSection id="lms-submit" number="03" label="Отчёт и LMS" title="Требования к отчёту и сдаче" icon={<GraduationCap aria-hidden="true" />}>
              <h3>Критерии оценки · {lab.points} баллов</h3><ResponsiveTable data={{columns:['Результат','Баллы'],rows:lab.rubric.map(r=>[r.criterion,r.points])}}/><p>Каждый критерий оценивается независимо. Частично подтверждённый результат получает соответствующую долю баллов внутри критерия; одна и та же проверка не засчитывается дважды.</p><p><strong>Один заполненный редактируемый DOCX-файл.</strong></p><h3>Требования к Word-файлу</h3><Checklist items={lab.wordRequirements.map(labText)} />
            ...
            <nav className="lab-pager" aria-label="Соседние лабораторные работы">
              {previous ? <a href={`#/lab/${previous.slug}`}><ArrowLeft aria-hidden="true" /> <span><small>Предыдущая</small>ЛР {previous.number} · {previous.semester} семестр</span></a> : <span />}
              {next ? <a href={`#/lab/${next.slug}`}><span><small>Следующая</small>ЛР {next.number} · {next.semester} семестр</span> <ArrowRight aria-hidden="true" /></a> : <span />}
            </nav>
          </article>
...
async function teacherBundle(selected:Lab[]){
 ...
 return downloadAllVariants(subjectAreas.map(area=>{const profile=profiles.find(p=>p.id===area.profileId);if(!profile)throw Error('Не найден профиль варианта');return {labs:selected.map(l=>resolveLab(l,area.id)),area,profile,renderPage:(lab:Lab)=>renderToStaticMarkup(<LabPage lab={lab} subjectArea={area} profile={profile} onSubjectAreaChange={()=>{}} />)}}))
}
