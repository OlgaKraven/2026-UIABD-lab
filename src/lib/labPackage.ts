...
const fetchBytes=(path:string):Promise<Uint8Array>=>{const hit=cache.get(path);if(hit)return hit;const pending=readBytes(path).catch(e=>{cache.delete(path);throw e});cache.set(path,pending);return pending};
const readBytes=async(path:string)=>{const r=await fetch(import.meta.env.BASE_URL+path);if(!r.ok)throw Error(`Не найден файл ${path}`);return new Uint8Array(await r.arrayBuffer())}
...
async function buildFiles({labs,area,profile,renderPage}:{labs:Lab[];area:SubjectArea;profile:QualityProfile;renderPage:(lab:Lab)=>string}){
 ...
 for(const path of lab.standFiles){const section=lab.sourceData.sections.find(s=>s.title===path.split('/').at(-1));files[dir+'Стенд/'+path.replace(/^inputs\//,'')]=path.endsWith('.json')&&section?.content?strToU8(section.content.at(-1)||''):await fetchBytes(path);}
 for(const section of lab.sourceData.sections)if(section.title==='parameters.sql'&&section.content)files[dir+'Стенд/parameters.sql']=strToU8(section.content.at(-1)||'');
 ...
 const prerequisite=doc.createElement('p');prerequisite.textContent=labMethodology[lab.slug].sequence.previous;body.append(prerequisite);
 for(const id of ['goal','situation','profile','inputs','theory','example','task','self-check','lms-submit','sequence']){
